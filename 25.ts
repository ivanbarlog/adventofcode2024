...
const sum = instructions
  .map(({ buttonA, buttonB, price }) => {
    if (buttonA.x * maxButtonHit + buttonB.x * maxButtonHit < price.x) return 0;
    if (buttonA.y * maxButtonHit + buttonB.y * maxButtonHit < price.y) return 0;

    for (let a = 0; a < maxButtonHit; a++) {
      for (let b = 0; b < maxButtonHit; b++) {
        if (buttonA.x * a + buttonB.x * b !== price.x) continue;
        if (buttonA.y * a + buttonB.y * b !== price.y) continue;

        return a * pushATokensPrice + b * pushBTokensPrice;
      }
    }
