const input = `Button A: X+24, Y+90
Button B: X+85, Y+62
Prize: X=6844, Y=6152
...
const pushATokensPrice = 3;
const pushBTokensPrice = 1;
const priceOffset = 10_000_000_000_000;

const instructions = input.split("\n\n").map((instruction) => {
  const [buttonA, buttonB, prize] = instruction.split("\n");

  const [, xA, yA] = buttonA.match(/^Button A: X\+(\d+), Y\+(\d+)$/) ?? [];
  const [, xB, yB] = buttonB.match(/^Button B: X\+(\d+), Y\+(\d+)$/) ?? [];
  const [, xP, yP] = prize.match(/^Prize: X=(\d+), Y=(\d+)$/) ?? [];
  ...
  return {
    ...
    price: {
      x: Number(xP) + priceOffset,
      y: Number(yP) + priceOffset,
    },
  };
});

const sum = instructions
  .map(({ buttonA, buttonB, price }) => {
    // 1. we have following equations - we want to get to aPresses and bPresses:
    // price.x = aPresses * buttonA.x + bPresses * buttonB.x
    // price.y = aPresses * buttonA.y + bPresses * buttonB.y
    //
    // 2. we simplify the equations and try to get the aPresses:
    // aPresses = (price.x - bPresses * buttonB.x) / buttonA.x
    // aPresses = (price.y - bPresses * buttonB.y) / buttonA.y
    //
    // 3. we join two equations to one with only one unknown - bPresses:
    //                        (price.x - bPresses * buttonB.x) / buttonA.x = (price.y - bPresses * buttonB.y) / buttonA.y
    //                        buttonA.y * (price.x - bPresses * buttonB.x) = buttonA.x * (price.y - bPresses * buttonB.y)
    //              buttonA.y * price.x - buttonA.y * bPresses * buttonB.x = buttonA.x * price.y - buttonA.x * bPresses * buttonB.y
    // buttonA.x * bPresses * buttonB.y - buttonA.y * bPresses * buttonB.x = buttonA.x * price.y - buttonA.y * price.x
    //          bPresses * (buttonA.x * buttonB.y - buttonA.y * buttonB.x) = buttonA.x * price.y - buttonA.y * price.x
    //                                                            bPresses = (buttonA.x * price.y - buttonA.y * price.x) / (buttonA.x * buttonB.y - buttonA.y * buttonB.x)
    //
    // 4. we calculate aPresses by substituting already known bPresses

    const bPresses =
      (buttonA.x * price.y - buttonA.y * price.x) /
      (buttonA.x * buttonB.y - buttonA.y * buttonB.x);
    const aPresses = (price.x - bPresses * buttonB.x) / buttonA.x;

    if (aPresses !== Math.floor(aPresses)) return 0;
    if (bPresses !== Math.floor(bPresses)) return 0;

    if (aPresses * buttonA.x + bPresses * buttonB.x !== price.x) return 0;
    if (aPresses * buttonA.y + bPresses * buttonB.y !== price.y) return 0;

    return aPresses * pushATokensPrice + bPresses * pushBTokensPrice;
  })
