...
function processTrailhead(coordinate: Coordinate) {
  const ends = new Set<`${number}:${number}`>();
  ...
  endCoordinates.forEach(({ x, y }) => ends.add(`${x}:${y}`));

  scores.push(ends.size);
}
...
const result = scores.reduce((result, current) => result + current, 0);
