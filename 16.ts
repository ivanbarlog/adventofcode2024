...
function addAntiNodeCoordinates(props: {
  node: { x: number; y: number };
  xDiff: number;
  yDiff: number;
}) {
  if (!canAddAntiNode(props.node)) return;
  antiNodes.add(`${props.node.x}:${props.node.y}`);

  const node = toAntiNodeCoordinate(props);
  addAntiNodeCoordinates({
    ...props,
    node,
  });
